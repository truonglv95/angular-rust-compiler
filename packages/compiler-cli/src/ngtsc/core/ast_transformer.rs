...
fn add_hoisted_statements<'a>(
    ...
    if parse_result.errors.is_empty() {
        ...
        let last_import_index = program.body.iter()
...
fn create_static_property<'a>(
    ...
    let template = allocator.alloc_str(&format!("class _ {{ static {} = {}; }}", prop_name, expr_str));
    let source_type = SourceType::mjs();
                ...
                for element in &class_decl.body.body {
                    if let ClassElement::PropertyDefinition(prop) = element {
                        ...
                        unsafe {
                            ...
                            let prop_ptr = prop as *const oxc_allocator::Box<'a, PropertyDefinition<'a>>;
...
fn ensure_angular_core_import<'a>(
    ...
    if !has_i0_import {
        ...
        let import_decl = ast.module_declaration_import_declaration(
            SPAN,
            Some(ast.vec1(ImportDeclarationSpecifier::ImportNamespaceSpecifier(
