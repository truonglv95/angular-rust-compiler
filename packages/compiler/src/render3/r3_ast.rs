...
pub enum R3Node {
    Comment(Box<Comment>),
    Text(Box<Text>),
    BoundText(Box<BoundText>),
    TextAttribute(Box<TextAttribute>),
    BoundAttribute(Box<BoundAttribute>),
    BoundEvent(Box<BoundEvent>),
    Element(Box<Element>),
    DeferredTrigger(Box<DeferredTrigger>),
    DeferredBlockPlaceholder(Box<DeferredBlockPlaceholder>),
    DeferredBlockLoading(Box<DeferredBlockLoading>),
    DeferredBlockError(Box<DeferredBlockError>),
    DeferredBlock(Box<DeferredBlock>),
    SwitchBlock(Box<SwitchBlock>),
    SwitchBlockCase(Box<SwitchBlockCase>),
    ForLoopBlock(Box<ForLoopBlock>),
    ForLoopBlockEmpty(Box<ForLoopBlockEmpty>),
    IfBlock(Box<IfBlock>),
    IfBlockBranch(Box<IfBlockBranch>),
    UnknownBlock(Box<UnknownBlock>),
